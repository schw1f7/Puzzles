...
public class PuzzleState implements State {
    ...
    public int hashCode() {
        int hash = 7;
        hash = 41 * hash + Arrays.deepHashCode(this.tiles);
        return hash;
    }
    ...
    */
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        for(int i = 1; i < 9; i++){
            if(!(curr.getLocation(i).equals(g.getLocation(i))))
                count++;
        }
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        for(int i = 1; i < 9; i++){
            if(!(curr.getLocation(i).equals(g.getLocation(i)))){
                sum += Math.abs(g.getLocation(i).getRow()- curr.getLocation(i).getRow()) + Math.abs(g.getLocation(i).getColumn() - curr.getLocation(i).getColumn());
            }
        }
        return sum;
    }
